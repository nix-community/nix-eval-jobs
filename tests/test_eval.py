...
def test_empty_needed() -> None:
    ...
    with TemporaryDirectory() as tempdir:
        ...
        assert any(proxy_wrapper_result["drvPath"] in drv for drv in web_service_result["neededBuilds"])

        # proxyWrapper should have nginx in its neededSubstitutes
        assert len(proxy_wrapper_result["neededSubstitutes"]) > 0
        assert any(nginx_result["outputs"]["out"] in out for out in proxy_wrapper_result["neededSubstitutes"])
